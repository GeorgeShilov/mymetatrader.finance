...
- for this project, the domain is `mymetatrader.finance`
...
```text
https://web.archive.org/web/20160328162852/mymetatrader.finance/
```
...
- Railway project: `mymetatrader.finance`
- Railway service: `mymetatrader.finance`
- Railway temporary domain: `https://codexworkplacecom-production.up.railway.app`
- Custom domain: `mymetatrader.finance`
...
```bash
railway init -n mymetatrader.finance -w <workspace-id> --json
railway add --service mymetatrader.finance --json
railway up --service mymetatrader.finance --detach --verbose
railway domain --service mymetatrader.finance --json
railway variable set SITE_URL=https://mymetatrader.finance --service mymetatrader.finance --environment production --skip-deploys
```

If `railway domain mymetatrader.finance ...` returns `Unauthorized` even after `railway login`, use the Railway dashboard or Railway GraphQL API to create the custom domain. In this run, the CLI was intermittently authenticated for `status` but unauthorized for `domain`.
...
Railway gave these records for `mymetatrader.finance`:
...
Zone > DNS > Edit
Zone > Zone > Read
Zone Resources: Include > Specific zone > mymetatrader.finance
```
...
const zoneName = "mymetatrader.finance";
...
```bash
curl -s "https://cloudflare-dns.com/dns-query?name=mymetatrader.finance&type=A" \
  -H "accept: application/dns-json"

curl -s "https://cloudflare-dns.com/dns-query?name=_railway-verify.mymetatrader.finance&type=TXT" \
  -H "accept: application/dns-json"
```
